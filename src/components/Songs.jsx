      ...
      <div className="flex flex-row row-span-11 justify-center gap-10 flex-wrap  h-full overflow-auto">
        { trackData.map(item => 
            <div key={item.id} className="max-w-[300px]">
              <Link to={`${item.id}?name=${item.name}&artist=${item.artists[0].name}&image=${item.album.images[1].url}`}>
                <img src={`${item.album.images[1].url}`} className="rounded-xl mb-2"></img>
              </Link>
              <p>Track: {item.name}</p>
              <p>Artist: {item.artists[0].name}</p>
              <p>Rating: {item.popularity}</p>
              <a href={`${item.preview_url}`} target="_" className="">
                <button className="rounded-xl w-fit h-12 bg-[#6C733D] p-2 hover:bg-[#9DA65D] hover:text-white">Play</button>
              </a>
            </div> 
        )}
      </div>
